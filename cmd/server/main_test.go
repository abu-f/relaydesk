...
func TestAvailableProxyCacheInvalidatesOnProxyStateChange(t *testing.T) {
	a := testApp(t)
	p, err := parseProxy("http://127.0.20.1:3128")
	if err != nil {
		t.Fatal(err)
	}
	id, err := a.insertProxy(p)
	...
	if err != nil || len(proxies) != 1 {
		t.Fatalf("expected cached proxy: proxies=%#v err=%v", proxies, err)
	}
	if err := a.db.QueryRow("SELECT COUNT(*) FROM proxies").Scan(new(int)); err != nil {
		t.Fatal(err)
	}
	...
	if len(proxies) != 0 {
		t.Fatalf("cooling proxy remained in the available cache: %#v", proxies)
	}
}

func TestProxyClientCacheReusesTransportUntilConfigurationChanges(t *testing.T) {
	a := testApp(t)
	p, err := parseProxy("http://127.0.20.2:3128")
	if err != nil {
		t.Fatal(err)
	}
	p.ID = 42
	first, err := a.httpClient(p)
	if err != nil {
		t.Fatal(err)
	}
	second, err := a.httpClient(p)
	if err != nil {
		t.Fatal(err)
	}
	if first != second {
		t.Fatal("expected proxy client cache to reuse the existing transport")
	}
	a.invalidateProxyCache()
	third, err := a.httpClient(p)
	if err != nil {
		t.Fatal(err)
	}
	if first == third {
		t.Fatal("expected proxy configuration invalidation to create a new client")
	}
}
